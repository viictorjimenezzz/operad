...
export function CurveOverlay({ series, isHeterogeneous = false, height = 280 }: CurveOverlayProps) {
  const nonEmpty = series.filter((s) => s.points.length > 0);
  if (nonEmpty.length === 0) {
    return (
      <EmptyState title="no curve data" description="selected runs have no comparable curve" />
    );
  }
