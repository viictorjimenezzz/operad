...
export function SweepHeatmap({ data }: { data: unknown }) {
  ...
  if (dims > 5) {
    return <EmptyState title="too many dimensions" description="view as table in the Cells tab" />;
  }
  if (dims <= 1) {
    ...
    <div className="overflow-auto">
      <table className="border-collapse text-[11px]">
        ...
        <tbody>
          {ax0.values.map((v0) => (
            <tr key={String(v0)}>
              <td className="sticky left-0 bg-bg-1 px-2 py-1 font-mono text-text">{String(v0)}</td>
              {ax1.values.map((v1) => {
                const entry = cellMap.get(`${v0}||${v1}`);
                const score = entry?.score ?? null;
                const alpha = score !== null ? 0.08 + ((score - scoreMin) / range) * 0.62 : 0;
  ...
  if (!ax2) {
    return (
      <EmptyState
        ...
        description="small multiples require a third axis"
      />
    );
  }
