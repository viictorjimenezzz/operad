...
export function MultiPromptDiff({ prompts }: MultiPromptDiffProps) {
  if (prompts.length < 2) {
    return <EmptyState title="not enough prompts" description="select at least two runs to compare prompt text" />;
  }
      ...
      {prompts.map((p) => {
        ...
        return (
          <div key={p.runId} className="min-w-0 rounded border border-border bg-bg-2 p-2">
            <div className="mb-2 text-[0.68rem] uppercase tracking-[0.08em] text-muted">{p.label}</div>
