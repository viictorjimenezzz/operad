...
function cassetteHref(path: string): string {
  return `/cassettes/${path.split("/").map((seg) => encodeURIComponent(seg)).join("/")}`;
}
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-auto p-3">
        {items.length === 0 ? (
          <EmptyState title="no cassettes" description="set OPERAD_DASHBOARD_CASSETTE_DIR or add files to ./.cassettes" />
        ) : (
              ...
              return (
                <section key={item.path} className="rounded-md border border-border bg-bg-1 p-3">
                  <div className="mb-2 flex items-center gap-2">
                    <Link to={cassetteHref(item.path)} className="font-mono text-xs text-text hover:text-accent">
                      {item.path}
                    </Link>
