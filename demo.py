...
Override with OPERAD_LLAMACPP_HOST / OPERAD_LLAMACPP_MODEL.
"""
...
async def _main() -> int:
    ...
    if not _server_reachable(cfg_host):
        console.print(
    ...
    # Stage 5 — mutation + diff.
    mutated = copy.deepcopy(agent)
    AppendRule(path="", rule="Be terse.").apply(mutated)
    console.print(Panel(str(agent.diff(mutated)), title="5. Mutation diff", border_style="cyan"))
