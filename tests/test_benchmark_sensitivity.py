...
async def test_baseline_matches_evaluate(cfg) -> None:
    ...
    report = await sensitivity(
        ...
        ds,
        metric,
        perturbations={"config.temperature": [0.1]},
    )
...
async def test_explicit_perturbation_cells_present(cfg) -> None:
    ...
    report = await sensitivity(
        ...
        ds,
        ValueMetric(),
        perturbations={"config.temperature": [0.1, 0.9]},
    )

    assert isinstance(report, SensitivityReport)
    temp_cells = [c for c in report.cells if c.parameter == "config.temperature"]
...
async def test_ranking_orders_by_abs_delta(cfg) -> None:
    ...
    config = cfg.model_copy(update={"top_p": 0.5})
    ...
    report = await sensitivity(
        ...
        ValueMetric(),
        perturbations={
            "config.temperature": [0.51, 0.52],
            "config.top_p": [0.2, 0.9],
        },
    )

    assert report.ranking[0][0] == "config.top_p"
    assert report.ranking[1][0] == "config.temperature"
    top_delta = next(d for p, d in report.ranking if p == "config.top_p")
    temp_delta = next(d for p, d in report.ranking if p == "config.temperature")
...
async def test_ranking_tie_break_on_path_string(cfg) -> None:
    ...
    report = await sensitivity(
        ...
        perturbations={
            ...
            "config.temperature": [0.1, 0.2],
        },
    )

    paths = [p for p, _ in report.ranking]
    assert paths == ["config.temperature", "config.top_p"]


async def test_default_perturbations_probes_sampling_axes(cfg) -> None:
    config = cfg.model_copy(update={"temperature": 0.5, "top_p": 0.5, "top_k": 40})
    agent = FakeLeaf(
...
async def test_concurrency_one_serialises_evaluations(cfg) -> None:
    ...
    await sensitivity(
        ...
        ds,
        ValueMetric(),
        perturbations={"config.temperature": [0.1, 0.5, 0.9]},
        concurrency=1,
    )
...
async def test_agent_unchanged_after_sensitivity(cfg) -> None:
    config = cfg.model_copy(update={"temperature": 0.3})
    agent = FakeLeaf(
...
async def test_max_combinations_cap(cfg) -> None:
    ...
    with pytest.raises(ValueError, match="exceeding max_combinations"):
        await sensitivity(
            ...
            ds,
            ValueMetric(),
            perturbations={"config.temperature": [0.1, 0.2, 0.3]},
            max_combinations=2,
        )
