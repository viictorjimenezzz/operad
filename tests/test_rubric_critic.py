"""Tests for RubricCritic (LLM-judge-as-metric wrapper)."""
...
from operad import Candidate, Metric, RubricCritic, Score
...
async def test_rubric_critic_score_returns_score_field(cfg) -> None:
    critic = FakeLeaf(
        config=cfg,
        input=Candidate,
        output=Score,
    ...
    await critic.abuild()
    m = RubricCritic(critic=critic)
    assert await m.score(A(text="anything"), A(text="anything")) == 0.7


async def test_rubric_critic_score_batch_preserves_order(cfg) -> None:
    critic = FakeLeaf(
        config=cfg,
        input=Candidate,
        output=Score,
...
async def test_rubric_critic_score_batch_runs_concurrently(cfg) -> None:
...
async def test_rubric_critic_is_runtime_checkable(cfg) -> None:
    critic = FakeLeaf(
        config=cfg, input=Candidate, output=Score, canned={"score": 0.1},
    )
    await critic.abuild()
    assert isinstance(RubricCritic(critic=critic), Metric)
