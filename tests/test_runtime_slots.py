...
async def test_limit_1_serializes_two_concurrent_acquires() -> None:
    ...
    reg.set_limit(backend="llamacpp", host="127.0.0.1:1", limit=1)
    order: list[str] = []
    ...
    # Because limit=1, b can't enter until a exits.
...
async def test_set_limit_after_first_use_raises() -> None:
    ...
    with pytest.raises(RuntimeError, match="already in use"):
        reg.set_limit(backend="llamacpp", host="127.0.0.1:2", limit=2)


async def test_acquire_uses_global_registry() -> None:
    registry.reset()
    registry.set_limit(backend="llamacpp", host="127.0.0.1:3", limit=1)
    order: list[str] = []
